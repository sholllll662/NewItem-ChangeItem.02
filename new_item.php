  ...
  <div class='login-box' style="margin-top: 10px;">
    <div class='login-logo'>
      ...
      <button data-text="Awesome" class="button">
        <span class="actual-text">&nbsp;NEW&nbsp;ITEM&nbsp;MASTER&nbsp;</span>
        <span class="hover-text" aria-hidden="true">&nbsp;NEW&nbsp;ITEM&nbsp;MASTER&nbsp;</span>
      </button>
    </div>
    <!-- /.login-logo -->
      ...
      if (isset($_POST['login'])) {
        ...
        if (mysqli_num_rows($query) == 0) {
          ...
          if ($row['id_level'] == 1 && $id_level == 1) {
            ...
            header("Location: dash_fm");
          } else if ($row['id_level'] == 2 && $id_level == 2) {
            $_SESSION['username'] = $username;
            $_SESSION['nama']     = $row['nama'];
            $_SESSION['id']     = $row['id'];
            $_SESSION['id_level'] = 'IT';
            header("Location: dash_it");
          } else if ($row['id_level'] == 3 && $id_level == 3) {
            $_SESSION['username'] = $username;
            $_SESSION['nama']     = $row['nama'];
            $_SESSION['id']     = $row['id'];
            $_SESSION['id_level'] = 'GM MKT';
            header("Location: dash_gm");
          } else if ($row['id_level'] == 4 && $id_level == 4) {
            $_SESSION['username'] = $username;
            $_SESSION['nama']     = $row['nama'];
            $_SESSION['id']     = $row['id'];
            $_SESSION['id_level'] = 'PPIC';
            header("Location: dash_ppic02");
          } else if ($row['id_level'] == 5 && $id_level == 5) {
            $_SESSION['username'] = $username;
            $_SESSION['nama']     = $row['nama'];
            $_SESSION['id']     = $row['id'];
            $_SESSION['id_level'] = 'QC DEPT. HEAD';
            header("Location: dash_qchead");
          } else if ($row['id_level'] == 9 && $id_level == 9) {
            $_SESSION['username'] = $username;
            $_SESSION['nama']     = $row['nama'];
            $_SESSION['id']     = $row['id'];
            $_SESSION['id_level'] = 'ENGINEERING';
            header("Location: dash_eng");
          } else if ($row['id_level'] == 10 && $id_level == 10) {
            $_SESSION['username'] = $username;
            $_SESSION['nama']     = $row['nama'];
            $_SESSION['id']     = $row['id'];
            $_SESSION['id_level'] = 'MARKETING';
            // $_SESSION['logged_in'] = true;
            // // Set cookie yang akan kedaluwarsa setelah 
            // setcookie('user_login', 'true', time() + 30);
            // if (!isset($_SESSION['logged_in']) || $_SESSION['logged_in'] !== true) {
            //   if (!isset($_COOKIE['user_login']) || $_COOKIE['user_login'] !== 'true') {
            //     // Redirect user ke halaman login
            //     header('Location: dash_mkt');
            //     exit;
            //   }
            // }
            header("Location: dash_mkt");
