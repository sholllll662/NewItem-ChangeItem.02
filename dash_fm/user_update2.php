...
$keputusan = 'Approved by GM HRD IT';
...
if ($id == null) {
    echo "<script onscreen>alert('HARAP UNTUK MEMILIH DATA YANG INGIN DI APPROVE!')</script>";
    echo "<center><a href='view-data2' onclick='return confirm('Are you sure?');'> KEMBALI </a><center>";
} else {
    $id = $_POST['selector'];
    $N = count($id);
    for ($i = 0; $i < $N; $i++) {
        mysqli_query($koneksi, "UPDATE newitem SET keputusan='$keputusan' WHERE id='$id[$i]'");
    }
